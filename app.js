// app.js  (ES module) — FIXED
// - Removed unauthenticated read of /users/{id}
// - Availability relies on /userIDs/{id}
// ================================
...
export async function sha(input){
  ...
  // Fallback SHA-256 (omitted for brevity)
  function R(n, x){ return (x>>>n) | (x<<(32-n)); }
  function toWords(bytes){ const words=[]; for(let i=0;i<bytes.length;i+=4){words.push((bytes[i]<<24)|(bytes[i+1]<<16)|(bytes[i+2]<<8)|(bytes[i+3]));} return words; }
  ...
  for(let i=0;i<bytes.length;i+=64){const chunk=bytes.slice(i,i+64);const w=new Array(64);const words=toWords(chunk);for(let t=0;t<16;t++)w[t]=words[t];for(let t=16;t<64;t++){const s0=((w[t-15]>>>7)|(w[t-15]<<25))^((w[t-15]>>>18)|(w[t-15]<<14))^(w[t-15]>>>3);const s1=((w[t-2]>>>17)|(w[t-2]<<15))^((w[t-2]>>>19)|(w[t-2]<<13))^(w[t-2]>>>10);w[t]=(w[t-16]+s0+w[t-7]+s1)|0;}let a=H0,b=H1,c=H2,d=H3,e=H4,f=H5,g=H6,h=H7;for(let t=0;t<64;t++){const S1=((e>>>6)|(e<<26))^((e>>>11)|(e<<21))^((e>>>25)|(e<<7));const ch=(e&f)^(~e&g);const temp1=(h+S1+ch+K[t]+w[t])|0;const S0=((a>>>2)|(a<<30))^((a>>>13)|(a<<19))^((a>>>22)|(a<<10));const maj=(a&b)^(a&c)^(b&c);const temp2=(S0+maj)|0;h=g;g=f;f=e;e=(d+temp1)|0;d=c;c=b;b=a;a=(temp1+temp2)|0;}H0=(H0+a)|0;H1=(H1+b)|0;H2=(H2+c)|0;H3=(H3+d)|0;H4=(H4+e)|0;H5=(H5+f)|0;H6=(H6+g)|0;H7=(H7+h)|0;}
...
export async function addUser(username, password, userId){
  ...
  // ✅ Only check /userIDs/{id} (allowed to read unauthenticated)
